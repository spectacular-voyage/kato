...
Current triage: 4 follow-ups look actionable, 1 comment is a Dendron false
positive and should stay cancelled.

Inline comments:
In `@apps/runtime/src/workspace/registry.ts`:
- [ ] Around line 94-95: The workspace equality checks in registry.ts currently
...
- [ ] Around line 620-645: The display name enrichment only looks up
workspaceDisplayName from workspaceDisplayNamesById (keyed by workspaceId) so
live-only rows that have workspaceAlias but no workspaceId remain un-enriched;
create a second map from workspaceEntries keyed by alias (e.g.,
workspaceDisplayNamesByAlias) and update the displayRecordings mapping (the
block that maps over filteredRecordings and sets workspaceDisplayName) to fall
back to that alias map when recording.workspaceId is falsy but
recording.workspaceAlias exists (i.e., set workspaceDisplayName =
workspaceDisplayNamesByAlias.get(recording.workspaceAlias) when needed); keep
using workspaceDisplayNamesById for the primary lookup and ensure you reference
buildRecordingRows, normalizedMetadataList, and workspaceAlias in the change.

---

Nitpick comments:
In `@apps/cli/src/usage.ts`:
- [ ] Around line 95-101: Update the usage synopsis string for the "kato workspace
...
In `@apps/web/src/loaders/sessions.ts`:
- [ ] Around line 599-605: The page assembly reads the workspace registry multiple
