...
const GLOBAL_USAGE_BODY = [
  ...
  "  detach <session-id>   Clear a session workspace attachment",
...
const COMMAND_USAGE_BODY: Record<DaemonCliCommandName, string> = {
  ...
  attach: [
    ...
    "",
    "Binds a session to the nearest ancestor .kato/kato-config.yaml.",
    "If --output is provided, it also updates the session's primary recording destination.",
  ].join("\n"),
  attachments: [
    "Usage: kato attachments [--all]",
    "",
    "Lists session workspace attachments.",
    "--all also includes sessions currently using the default workspace.",
  ].join("\n"),
  detach: [
    "Usage: kato detach <session-id>",
    "",
    "Clears a session's explicit workspace attachment.",
  ].join("\n"),
  export: [
