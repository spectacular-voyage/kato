...
const GLOBAL_USAGE_BODY = [
  ...
  "                        Show daemon status",
  "  web <init|start|restart|stop|status>",
...
const COMMAND_USAGE_BODY: Record<DaemonCliCommandName, string> = {
  ...
  web: [
    ...
    "`kato web start --host` overrides the configured hostname for this run only (e.g. --host 0.0.0.0 to listen on all interfaces).",
  ].join("\n"),
  "workspace-init": [
  ...
  "workspace-register": [
    "Usage: kato workspace register [<dir>] [--alias <alias>] [--name <display-name>] [--no-restart]",
    "",
    ...
    "If --name is provided, saves an operator-facing workspace label.",
